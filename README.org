...
- ~C-c C-e~ -- calls ~org-msg-preview~, it generates the final HTML email, saves it into a temporary file and calls the ~browse-url~ function on that file.
...
- ~C-c C-a~ -- calls ~org-msg-attach~, very similar to the ~org-attach~ function.  It lets you add or delete an attachment to this email.  The attachment list is stored in the ~:attachment:~ property.
- ~C-c C-c~ -- calls ~org-ctrl-c-ctrl-c~. *OrgMsg* configures ~org-msg-ctrl-c-ctrl-c~ as a final hook of [[https://orgmode.org/][Org mode]]. When ~org-msg-ctrl-c-ctrl-c~ is called in a *OrgMsg* buffer it generates the MIME message and sends it.

The ~org-msg-mode~ interactive function can be called to enable/disable *OrgMsg*.  By default, once the module is loaded, it is disabled.  If you want to reply to an email without making use of *OrgMsg*, you should call that function before you call the reply-to function.
...
If ~org-msg-text-plain-alternative~ is ~t~, *OrgMsg* generates an ASCII export of the message and attaches it as a ~text/plain~ alternative to the HTML email.
